# Vendored verbatim from ductuantruong/tcm_add/conformer.py (upstream conformer blocks).
...
class _RearrangeBNCtoBCN(nn.Module):
    def forward(self, x):
        return x.transpose(1, 2)


class _RearrangeBCNtoBNC(nn.Module):
    def forward(self, x):
        return x.transpose(1, 2)
...
class Attention(nn.Module):
    ...
    def __init__(self, dim, heads=8, dim_head=64, qkv_bias=False, dropout=0.0, proj_drop=0.0):
        super().__init__()
    ...
    def forward(self, x, mask=None):
        ...
        qkv = (
            self.qkv(x)
            .reshape(B, N + self.num_heads, 3, self.num_heads, C // self.num_heads)
            .permute(2, 0, 3, 1, 4)
        )
