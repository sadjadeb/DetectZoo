...
_CKPT_ZIP_URL = (
    f"https://nextcloud.eurecom.fr/s/{_NEXTCLOUD_SHARE_TOKEN}/download/{_DEFAULT_CKPT_NAME}"
)
_ZIP_MEMBER = f"Raw-PCDARTS/{_DEFAULT_CKPT_NAME}"
_MIN_CKPT_BYTES = 1024
...
def _checkpoint_is_valid(path: Path) -> bool:
    return path.is_file() and path.stat().st_size >= _MIN_CKPT_BYTES


def _download_nextcloud_checkpoint(cache_dir: Path) -> Path:
    """Download the official Nextcloud ZIP archive and extract ``fix_mel.pth``."""
    cached = cache_dir / _DEFAULT_CKPT_NAME
    zip_path = cache_dir / "raw_pc_darts_weights.zip"
    ...
    download_file(_CKPT_ZIP_URL, zip_path, force=True)

    with zipfile.ZipFile(zip_path) as archive:
        members = archive.namelist()
        member = _ZIP_MEMBER if _ZIP_MEMBER in members else None
        if member is None:
            matches = [name for name in members if name.endswith(_DEFAULT_CKPT_NAME)]
            if not matches:
                raise RuntimeError(
                    f"Raw-PC-DARTS: {_DEFAULT_CKPT_NAME} not found in Nextcloud archive. "
                    f"Members (first 10): {members[:10]}"
                )
            member = matches[0]
        with archive.open(member) as src, open(cached, "wb") as dst:
            dst.write(src.read())

    if not _checkpoint_is_valid(cached):
        raise RuntimeError(
            f"Raw-PC-DARTS: extracted checkpoint at {cached} is missing or too small."
        )
    return cached
...
def resolve_checkpoint_path(
    ...
    *,
    ...
    if _checkpoint_is_valid(cached):
        _LOGGER.info("Raw-PC-DARTS: using cached checkpoint %s", cached)
        return cached

    return _download_nextcloud_checkpoint(cache_dir)
