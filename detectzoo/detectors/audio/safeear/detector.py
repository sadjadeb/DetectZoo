...
class SafeEarDetector(BaseDetector):
    ...
    def _ensure_loaded(self) -> None:
        ...
        print(
            f"SafeEar load report: codec matched={report.get('codec_matched', '?')}, "
            f"codec missing={report.get('codec_missing', '?')}; "
            f"detect matched={report.get('detect_matched', '?')}, "
            f"detect missing={report.get('detect_missing', '?')}"
        )
