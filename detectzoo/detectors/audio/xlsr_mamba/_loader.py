...
def build_xlsr_mamba_model(
    ...
    *,
    ...
    # Fused Triton norms require the optional ``mamba-ssm`` CUDA stack. The
    # vendored fallback uses eager LayerNorm (same weights, slower on CPU).
    try:
        from mamba_ssm.ops.triton.layernorm import layer_norm_fn as _ln  # noqa: F401

        fused_add_norm = torch.cuda.is_available()
    except ImportError:
        fused_add_norm = False
    if not fused_add_norm:
        _LOGGER.info("XLSR-Mamba: using fused_add_norm=False (vendored or CPU path).")
