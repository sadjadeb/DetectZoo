...
Dependencies
------------
Requires ``mamba-ssm`` with a CUDA build matching your PyTorch wheel.
Install with ``pip install detectzoo[audio,xlsr_mamba]`` after installing
PyTorch. CPU-only hosts can run inference with ``fused_add_norm=False``
(fallback path) but still need ``mamba-ssm`` installed.
"""
...
class XLSRMambaDetector(BaseDetector):
    ...
    def _ensure_loaded(self) -> None:
        ...
        print(
            f"XLSR-Mamba load report ({self.model_name}): "
